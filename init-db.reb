Rebol [
    file: %init-db.reb
    purpose: -[
        Clears existing tables if present and then loads with some initial data
        ready for importing.
    ]-
    date: 3-Dec-2021
    author: "Graham Chiu"
...
drop-table: func [tablename [text!]] [
    sql-execute [DROP TABLE $[<!> tablename]]  ; note: can't use $tablename
]
...
if interactive? [
    if equal? "Yes" ask ["Delete all the data from patients database? (Yes/No) " text!] [
        ...
        for-each 'table [
            ...
            sys.util/recover [  ; !!! ODBC-EXECUTE errors not definitional ATM
