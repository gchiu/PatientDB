...
cmd: unspaced [{select id, filename from files where done = } #[false] "}"]
?? cmd
sql-execute cmd
...
for-each record records [; records contains all id, filenames from files where flag done is false
	...
	if exists? to file! join dir filename [
		...
		if parse? filename [nhi "-" copy clinician some further alpha thru "-" copy ldate 8 digit "-" to ".txt" to end] [
			; GChiu, Elasir
