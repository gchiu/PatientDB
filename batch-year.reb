Rebol [
    ...
    author: "Graham Chiu"
    purpose: --[
        Setup all the 3 years directories with all the docx files already
        converted to txt and then calls read-files to grab all the file names,
        and then parse-letter to extract all the information in the letters
...
insist [append years n, n: me - 1, n = 2018]
...
for-each 'year years [
    for-each 'cmonth months [
        ; do:args %read-files.reb  let dir: to file! unspaced ["/d/" year "/" year "/" cmonth "/"]
        do:args %read-files.reb  let dir: to file! unspaced [year "/" year "/" cmonth "/"]
